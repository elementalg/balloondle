...
/// <summary>
/// Acts as a touch detector that proceeds to move the joystick to the touched position, in order to make it easier for
...
public class JoystickSurface : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    /// <summary>
    /// Calculates the rectangle which contains all the possible points which can be used as center for the joystick.
    /// </summary>
    ...
    ///
    /// If the touch was not within the center bounds, it will proceed to update the joystick's pointer down position
    ...
    private bool UpdateJoystickCenterPosition(PointerEventData eventData)
    {
        ...
        else
        {
            ...
            Vector3 rootClosestPosition = transform.parent.InverseTransformPoint(worldClosestPositionToCenterBounds);
